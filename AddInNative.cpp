...
static unsigned int _stdcall RecvInThread(void* p)
{
	...
	while (tcpCl->connected && tcpCl->listen)
	{
		...
		if (recived > 0)
		{
			...
			while ((cutAt != s1.npos )&& (cutAt > 0)) {

				std::string nstr(s1.substr(0, cutAt + SEPLN.length()));
				wchar_t* res = CHAR_2_WCHAR((char*)nstr.c_str());

				nstr.clear();
				nstr.shrink_to_fit();
				size_t cut = cutAt + SEPLN.length();
				cutAt = 0;
				std::string(s1.substr(cut)).swap(s1);
			//	s1 = s1.substr(cut);
			//	s1.shrink_to_fit();
				...
				if (tcpCl->regEx != L"")
				{
					...
					try
					{
						std::wregex r(tcpCl->regEx);




						if (std::regex_search(res, r))
						{
					...
					catch (const std::regex_error& e)
					{
						...
						case std::regex_constants::error_collate:
							tcpCl->SendEvent(L"Некорректная Строка Regex", L"error_collate");
							break;
						case std::regex_constants::error_ctype:
							tcpCl->SendEvent(L"Некорректная Строка Regex", L"error_ctype");
							break;
						case std::regex_constants::error_escape:
							tcpCl->SendEvent(L"Некорректная Строка Regex", L"error_escape");
							break;
						case std::regex_constants::error_backref:
							tcpCl->SendEvent(L"Некорректная Строка Regex", L"error_backref");
							break;
						case std::regex_constants::error_brack:
							tcpCl->SendEvent(L"Некорректная Строка Regex", L"error_brack");
							break;
						case std::regex_constants::error_paren:
							tcpCl->SendEvent(L"Некорректная Строка Regex", L"error_paren");
							break;
						case std::regex_constants::error_brace:
							tcpCl->SendEvent(L"Некорректная Строка Regex", L"error_brace");
							break;
						case std::regex_constants::error_badbrace:
							tcpCl->SendEvent(L"Некорректная Строка Regex", L"error_badbrace");
							break;
						case std::regex_constants::error_range:
							tcpCl->SendEvent(L"Некорректная Строка Regex", L"error_range");
							break;
						case std::regex_constants::error_space:
							tcpCl->SendEvent(L"Некорректная Строка Regex", L"error_space");
							break;
						case std::regex_constants::error_badrepeat:
							tcpCl->SendEvent(L"Некорректная Строка Regex", L"error_badrepeat");
							break;
						case std::regex_constants::error_complexity:
							tcpCl->SendEvent(L"Некорректная Строка Regex", L"error_complexity");
							break;
						case std::regex_constants::error_stack:
							tcpCl->SendEvent(L"Некорректная Строка Regex", L"error_stack");
							break;
